...
func testUserToManyIDUserRecoveryCodes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, recoveryCodeDBTypes, false, recoveryCodeColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, recoveryCodeDBTypes, false, recoveryCodeColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.IDUserRecoveryCodes().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadIDUserRecoveryCodes(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserRecoveryCodes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDUserRecoveryCodes = nil
	if err = a.L.LoadIDUserRecoveryCodes(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserRecoveryCodes); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyIDUserRecoveryTokens(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, recoveryTokenDBTypes, false, recoveryTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, recoveryTokenDBTypes, false, recoveryTokenColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.IDUserRecoveryTokens().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadIDUserRecoveryTokens(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserRecoveryTokens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDUserRecoveryTokens = nil
	if err = a.L.LoadIDUserRecoveryTokens(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserRecoveryTokens); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpIDUserRecoveryCodes(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e RecoveryCode

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*RecoveryCode{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, recoveryCodeDBTypes, false, strmangle.SetComplement(recoveryCodePrimaryKeyColumns, recoveryCodeColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*RecoveryCode{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDUserRecoveryCodes(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDUserRecoveryCodes[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDUserRecoveryCodes[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDUserRecoveryCodes().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpIDUserRecoveryTokens(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e RecoveryToken

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*RecoveryToken{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, recoveryTokenDBTypes, false, strmangle.SetComplement(recoveryTokenPrimaryKeyColumns, recoveryTokenColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*RecoveryToken{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDUserRecoveryTokens(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDUserRecoveryTokens[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDUserRecoveryTokens[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDUserRecoveryTokens().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
