...
var UserRels = struct {
	IDUserAuth           string
	IDUserRecoveryCodes  string
	IDUserRecoveryTokens string
	IDUserRolesUsers     string
}{
	IDUserAuth:           "IDUserAuth",
	IDUserRecoveryCodes:  "IDUserRecoveryCodes",
	IDUserRecoveryTokens: "IDUserRecoveryTokens",
...
type userR struct {
	...
	IDUserRecoveryCodes  RecoveryCodeSlice  `boil:"IDUserRecoveryCodes" json:"IDUserRecoveryCodes" toml:"IDUserRecoveryCodes" yaml:"IDUserRecoveryCodes"`
	IDUserRecoveryTokens RecoveryTokenSlice `boil:"IDUserRecoveryTokens" json:"IDUserRecoveryTokens" toml:"IDUserRecoveryTokens" yaml:"IDUserRecoveryTokens"`
...
func (r *userR) GetIDUserRecoveryCodes() RecoveryCodeSlice {
	if r == nil {
		return nil
	}
	return r.IDUserRecoveryCodes
}

func (r *userR) GetIDUserRecoveryTokens() RecoveryTokenSlice {
	if r == nil {
		return nil
	}
	return r.IDUserRecoveryTokens
}
...
// IDUserRecoveryCodes retrieves all the recovery_code's RecoveryCodes with an executor via id_user column.
func (o *User) IDUserRecoveryCodes(mods ...qm.QueryMod) recoveryCodeQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"recovery_codes\".\"id_user\"=?", o.ID),
	)

	return RecoveryCodes(queryMods...)
}

// IDUserRecoveryTokens retrieves all the recovery_token's RecoveryTokens with an executor via id_user column.
func (o *User) IDUserRecoveryTokens(mods ...qm.QueryMod) recoveryTokenQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"recovery_tokens\".\"id_user\"=?", o.ID),
	)

	return RecoveryTokens(queryMods...)
}
...
// LoadIDUserRecoveryCodes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadIDUserRecoveryCodes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`recovery_codes`),
		qm.WhereIn(`recovery_codes.id_user in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load recovery_codes")
	}

	var resultSlice []*RecoveryCode
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice recovery_codes")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on recovery_codes")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for recovery_codes")
	}
	...
	if singular {
		object.R.IDUserRecoveryCodes = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &recoveryCodeR{}
			}
			foreign.R.IDUserUser = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.IDUser {
				local.R.IDUserRecoveryCodes = append(local.R.IDUserRecoveryCodes, foreign)
				if foreign.R == nil {
					foreign.R = &recoveryCodeR{}
				}
				foreign.R.IDUserUser = local
...
// LoadIDUserRecoveryTokens allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadIDUserRecoveryTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`recovery_tokens`),
		qm.WhereIn(`recovery_tokens.id_user in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load recovery_tokens")
	}

	var resultSlice []*RecoveryToken
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice recovery_tokens")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on recovery_tokens")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for recovery_tokens")
	}
	...
	if singular {
		object.R.IDUserRecoveryTokens = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &recoveryTokenR{}
			}
			foreign.R.IDUserUser = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.IDUser {
				local.R.IDUserRecoveryTokens = append(local.R.IDUserRecoveryTokens, foreign)
				if foreign.R == nil {
					foreign.R = &recoveryTokenR{}
				}
				foreign.R.IDUserUser = local
...
// AddIDUserRecoveryCodes adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.IDUserRecoveryCodes.
// Sets related.R.IDUserUser appropriately.
func (o *User) AddIDUserRecoveryCodes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RecoveryCode) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			IDUserRecoveryCodes: related,
		}
	} else {
		o.R.IDUserRecoveryCodes = append(o.R.IDUserRecoveryCodes, related...)
	}
...
// AddIDUserRecoveryTokens adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.IDUserRecoveryTokens.
// Sets related.R.IDUserUser appropriately.
func (o *User) AddIDUserRecoveryTokens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RecoveryToken) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.IDUser = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			IDUserRecoveryTokens: related,
		}
	} else {
		o.R.IDUserRecoveryTokens = append(o.R.IDUserRecoveryTokens, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &recoveryTokenR{
				IDUserUser: o,
			}
		} else {
			rel.R.IDUserUser = o
